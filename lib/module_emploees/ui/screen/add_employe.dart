import 'dart:async';
import 'package:flutter/material.dart';
import 'package:injectable/injectable.dart';
import 'package:oringe/abstracts/states/state.dart';
import 'package:oringe/generated/l10n.dart';
import 'package:oringe/module_emploees/request/create_employee_request.dart';
import 'package:oringe/module_emploees/state_manager/emplo_state_manager.dart';
import 'package:oringe/module_emploees/ui/state/add_employee_init.dart';
import 'package:tip_dialog/tip_dialog.dart';

@injectable
class AddEmployeeScreen extends StatefulWidget {
  final EmployeeStateManager _stateManager;

  AddEmployeeScreen(this._stateManager);

  @override
   AddEmployeeScreenState createState() =>  AddEmployeeScreenState();
}

class  AddEmployeeScreenState extends State<AddEmployeeScreen> {

  int id = -1;
  bool flags = true;
  late StreamSubscription _streamSubscription;
  late States currentState;

  void refresh() {
    if (mounted) {
      setState(() {});
    }
  }

  void createRecord(CreateRecordRequest request) {
    widget._stateManager.createRecord(this, request);
  }

  @override
  void initState() {
    super.initState();
    currentState = AddEmployeeInit(this);
    _streamSubscription = widget._stateManager.stateStream.listen((event) {
      currentState = event;
      if (mounted) {
        setState(() {});
      }
    });
  }
  @override
  Widget build(BuildContext context) {
    var args = ModalRoute.of(context)?.settings.arguments;
    if (args != null && flags) {
      if (args is int) id = args;
      flags = false;
    }
    return Scaffold(
      backgroundColor: Colors.grey.shade100,
      appBar: AppBar(
        backgroundColor: Colors.grey.shade200,
        surfaceTintColor: Colors.grey.shade200,
        centerTitle: true,
        elevation: 5,
//          shape: ShapeBorder.,
        title: Text(S.of(context).addRecord,
            style: TextStyle(
              fontSize: 25,
              fontWeight: FontWeight.bold,
            )),
      ),
        body: Stack(
          children: [
            currentState.getUI(context),
            TipDialogContainer(
                duration: const Duration(seconds: 2),
                outsideTouchable: true,
                onOutsideTouch: (Widget tipDialog) {
                  if (tipDialog is TipDialog &&
                      tipDialog.type == TipDialogType.LOADING) {
                    TipDialogHelper.dismiss();
                  }
                })

          ],
        ));
  }
}
